...
function parseHighestWindGust(...values: Array<string | null>) {
...
function createMorningCommuteRisk(weather: PredictionInput["weather"]) {
...
export function createPrediction({ weather, now = new Date() }: PredictionInput): PredictionResult {
  let score = 12;
  ...
  let label: PredictionResult["label"] = "Normal Operations Likely";
  if (cappedScore >= 72) {
